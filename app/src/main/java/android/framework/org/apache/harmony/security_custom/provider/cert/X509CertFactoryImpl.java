...
public class X509CertFactoryImpl extends CertificateFactorySpi {
    ...
    private static final byte[] PEM_BEGIN = "-----BEGIN".getBytes(Charset.forName("UTF-8"));
    private static final byte[] PEM_END = "-----END".getBytes(Charset.forName("UTF-8"));
    /**
     ...
     */
    private static final byte[] CERT_BOUND_SUFFIX = " CERTIFICATE-----".getBytes(Charset.forName("UTF-8"));
    ...
    private byte[] decodePEM(InputStream inStream, byte[] boundary_suffix)
        ...
        inStream.reset();
        buff = android.util.Base64.decode(buff, index);
        if (buff == null) {
